...
import { getEmotions } from "../../api/emotionService";
...
export default function ComicsPage() {
  ...
  /* ===== MAP emotionId -> emotionName ===== */
  const emotionMap = useMemo(() => {
    return emotions.reduce((map, e) => {
      map[e.id] = e.name;
      return map;
    }, {});
  }, [emotions]);
  ...
  const loadData = async () => {
    try {
      const [comicRes, emotionRes] = await Promise.all([
        getAllComics(),
        getEmotions(),
      ]);
      setComics(comicRes.data);
    ...
    <div className="mx-auto w-full max-w-screen-2xl px-4 py-8">
      ...
      <div className="mb-10 flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
        ...
        <button
          ...
          className="flex items-center gap-3 rounded-xl bg-gradient-to-r from-[#137fec] to-blue-600 px-6 py-4 font-bold text-white shadow-lg hover:shadow-xl transition"
        >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-8">
          {comics.map((comic) => {
            ...
            return (
              <div
                key={comic.id}
                className="flex flex-col rounded-2xl bg-white dark:bg-[#1A2633] shadow-lg overflow-hidden"
              >
                {/* COVER */}
                <div className="aspect-[3/4] overflow-hidden bg-gray-200 dark:bg-gray-800">
                  <img
                    src={
                      comic.coverImage ||
                      "https://via.placeholder.com/300x400/1A2633/94A3B8?text=No+Image"
                    }
                    alt={comic.title}
                ...
                <div className="flex flex-col flex-1 p-6 justify-between">
                  <div>
                    ...
                    <span
                      className={`inline-block rounded-full px-4 py-1.5 text-sm font-medium ${emotionClass}`}
                    >
                      {emotionName}
                    </span>
                  </div>
                  ...
                  <div className="mt-6 flex justify-between">
                    <button
                      onClick={() =>
                        navigate(`/admin/comics/${comic.id}/chapters`)
                      }
                      className="flex h-12 w-12 items-center justify-center rounded-xl bg-green-600 text-white hover:bg-green-700 shadow-md"
                    ...
                    <button
                      onClick={() =>
                        navigate(`/admin/comics/edit/${comic.id}`)
                      }
                      className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white shadow-md transition"
                      title="Sửa truyện"
                    >
                      <span className="material-symbols-outlined text-2xl">
                        edit
                    ...
                    <button
                      ...
                      className="flex h-12 w-12 items-center justify-center rounded-xl bg-red-100 text-red-600 hover:bg-red-600 hover:text-white shadow-md transition"
                      title="Xóa truyện"
                    >
                      <span className="material-symbols-outlined text-2xl">
                        delete
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-3xl bg-white dark:bg-[#1A2633] p-8 shadow-2xl">
            ...
            <p className="mb-8 text-[#617589] dark:text-[#94A3B8]">
              Xóa truyện{" "}
            ...
            <div className="flex justify-end gap-4">
              <button
                onClick={() => setDeleteTarget(null)}
                className="rounded-xl border px-6 py-3 hover:bg-gray-100 dark:hover:bg-[#2A3B4D]"
              >
                Hủy
              </button>
              <button
                onClick={handleDelete}
                className="rounded-xl bg-red-600 px-7 py-3 font-bold text-white hover:bg-red-700"
              >
                Xóa truyện
              </button>
            </div>
